//The fllowing 202 lines code was adapted from Garrit's lecture https://ju.slides.com/garrit/cc-2023-complexity?token=AbjTIZo5#/9/6
//The following 201 lines of code was added by courtesy of Garrit Schaap
...
class Agent {
	...
	// i changed here, when the lines touch the borders, it will generate random notes
	checkBorders() {
	  if (this.position.x < 0) {
		this.position.x = innerWidth;
		this.lastPosition.x = innerWidth;
		const synth = new Tone.Synth().toDestination();
		synth.triggerAttackRelease('C' + random(2,5	), "4n");
	  } else if (this.position.x > innerWidth) {
		this.position.x = 0;
		this.lastPosition.x = 0;
		const synth = new Tone.Synth().toDestination();
		synth.triggerAttackRelease('D' + random(2,4), "4n");
	  } 
	  if (this.position.y < 0) {
		this.position.y = innerHeight;
		this.lastPosition.y = innerHeight;
		const synth = new Tone.Synth().toDestination();
		synth.triggerAttackRelease('E' + random(2,4), "4n");
	  } else if (this.position.y > innerHeight) {
		this.position.y = 0;
		this.lastPosition.y = 0;
		const synth = new Tone.Synth().toDestination();
		synth.triggerAttackRelease('F' + random(2,4), "4n");
	  }
	}
